...
class VIReparamModel:
    def __init__(self, num_features):
        ...
        W, nu_mu, sigma, nu_pre_sigma = get_random_normal_variable('weight', [num_features + 1, ])
        ...
        # Do the multiplication
        activation = tf.matmul(data_ext, tf.expand_dims(W, axis=-1))  # Tensor in [batch_size, num_mc, ]

        # Use built in cross entropy from logits. It uses some tricks to prevent overflow near the boundaries
        self.log_likelihood = -tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=activation, labels=tf.expand_dims(self.targets, axis=-1)))

        # Take into account the prior
        variance_prior = 10**1
        self.log_prior = (num_features + 1) / 2 + tf.log(2 * np.pi * variance_prior) - 1 / (2 * variance_prior) * tf.reduce_sum(tf.pow(W, 2))

        # Get the posterior probability for our parameter
        self.log_p_theta_given_D = self.log_likelihood + self.log_prior
        ...
        gradient_mu = d_log_q_d_mu * elbo_instant
        gradient_pre_sigm = d_log_q_d_pre_sigma * elbo_instant

        gradients = [gradient_mu, gradient_pre_sigm]
        ...
        tf.summary.scalar("Log_likelihood", self.log_likelihood)
        tf.summary.scalar("Log_prior", self.log_prior)
        tf.summary.scalar("Accuracy", self.accuracy)
        tf.summary.scalar("ELBO_inst", elbo_instant)
...
#
#         log_q = tf.reduce_sum(- 1 / 2 * tf.log(2 * np.pi) - tf.log(sigma)
#                               - 1 / (2 * sigma ** 2) * tf.pow(W - nu_mu, 2))
#
#         # Do the multiplication
#         activation = tf.matmul(data_ext, tf.expand_dims(W, axis=-1))
#
#         # Use built in cross entropy from logits. It uses some tricks to prevent overflow near the boundaries
#         self.log_likelihood = -tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=activation, labels=tf.expand_dims(self.targets, axis=-1)))
#
...
#         # Set up a training schedule
#         global_step = tf.train.get_or_create_global_step()
#         learning_rate = tf.train.exponential_decay(0.000005, global_step, 500, 0.1)
#
#         # Write out the gradient
#         d_log_q_d_mu = tf.pow(sigma, -2) * (W - nu_mu)
#         d_log_q_d_pre_sigma = tf.pow(sigma, -1) * (tf.pow(sigma, -2)*tf.pow(W-nu_mu, 2) - 1) * tf.gradients(sigma, nu_pre_sigma)[0]
#
#         elbo_instant = self.log_likelihood + self.log_prior - log_q
#
#         # Keep consistent with other implementations and define tvars and gradients as lists
#         tvars = [nu_mu]  #, nu_pre_sigma]
#         gradients = [d_log_q_d_mu * elbo_instant]  #, -d_log_q_d_pre_sigma * elbo_instant]
#
#         self.train_step = tf.train.GradientDescentOptimizer(learning_rate).apply_gradients(zip(gradients, tvars))
#         self.init_op = tf.global_variables_initializer()
#
